...
print(accurate)
print(heatmap_list)
heatmap_list2 = heatmap_list
for yval,row in enumerate(heatmap_list):
    total = sum(row)
    for xval,part in enumerate(row):
        heatmap_list2[yval][xval] = round(part*100/total,2)
        
        
        
ax = sn.heatmap(heatmap_list2,cmap='Blues',xticklabels=models,yticklabels=models,annot=True,fmt='g')
plt.xlabel('Predicted type')
plt.ylabel('Ground-Truth')
# print(yhat)
for t in ax.texts: t.set_text(t.get_text() + " %")
# plt.show()
plt.savefig('/home/alex/Desktop/KURF/Scripts/Plots/heatmaplotsofvalues.png', dpi=400)
